"""Load handwritten Arabic-digit crops (local image+label pairs) for OCR training.

Expected layout:
    <root>/<split>/images/<stem>.png   (or .jpg)
    <root>/<split>/labels/<stem>.txt   (UTF-8, Eastern Arabic digits ٠١٢٣٤٥٦٧٨٩)

This is the same on-disk format as IFN/ENIT, but the labels are kept verbatim
(no dot-as-word-separator substitution), since digit strings don't use that
convention.
"""
...
def _load_arabic_digits_pairs(root_dir: str, split: str) -> list[dict[str, str]]:
    ...
    if not img_dir.is_dir() or not lbl_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Check that {root_dir}/{split}/images and labels folders are present."
        )
...
def build_arabic_digits_dataset(
    root_dir: str,
    split: str = "train",
    seed: int = 42,
    ...
    """Build a Qwen2-VL chat-formatted dataset from local Arabic-digit files."""
    pairs = _load_arabic_digits_pairs(root_dir, split)
    ...
    return ds.map(
        ...
        desc="Arabic digits augment + build messages",
    )
